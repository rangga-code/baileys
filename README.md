...
if (!Rafz.authState.creds.registered) {
    const number = 'XXXXXXXXXXX'
    const code = await Rafz.requestPairingCode(number) // or await Rafz.requestPairingCode(number, 'CODEOTPS') custom your pairing code
    console.log(code)
}
```
...
- If you use baileys for groups, we recommend you to set `cachedGroupMetadata` in socket config, you need to implement a cache like this:
    ...
    Rafz.ev.on('groups.update', async ([event]) => {
        const metadata = await Rafz.groupMetadata(event.id)
        groupCache.set(event.id, metadata)
    })

    Rafz.ev.on('group-participants.update', async (event) => {
        const metadata = await Rafz.groupMetadata(event.id)
        groupCache.set(event.id, metadata)
    })
    ```
...
const Rafz = makeWASocket({ auth: state })

// this will be called as soon as the credentials are updated
Rafz.ev.on('creds.update', saveCreds)
```
...
```ts
const Rafz = makeWASocket()
Rafz.ev.on('messages.upsert', ({ messages }) => {
...
async function connectToWhatsApp () {
    ...
    Rafz.ev.on('connection.update', (update) => {
    ...
    Rafz.ev.on('messages.upsert', event => {
        for (const m of event.messages) {
            console.log(JSON.stringify(m, undefined, 2))

            console.log('replying to', m.key.remoteJid)
            await Rangga.sendMessage(m.key.remoteJid!, { text: 'Hello Word' })
        }
    })
...
Rafz.ev.on("messages.update", async ([chatUpdate]) => {
    ...
    const agregate = getAggregateResponsesInEventMessage({ eventResponses }, jidNormalizedUser(Rafz.user.lid)) 
...
Rafz.ev.on('chats.upsert', () => {
    // can use 'store.chats' however you want, even after the socket dies out
    // 'chats' => a KeyedDB instance
    console.log('got chats', store.chats.all())
})

Rafz.ev.on('contacts.upsert', () => {
...
- Send all types of messages with a single function
    ...
    Rangga.sendMessage(jid, content, options)
    ```
...
#### Text Message
```ts
await Rangga.sendMessage(jid, { text: 'hello word' })
```

#### Quote Message (works with all types)
```ts
await Rangga.sendMessage(jid, { text: 'hello word' }, { quoted: message })
```

#### Mention User (works with most types)
- @number is to mention in text, it's optional
```ts
await Rangga.sendMessage(
    jid,
    {
        text: '@12345678901',
...
await Rangga.sendMessage(jid, { forward: msg, force: true or number }) // WA forward the message!
```
...
await Rangga.sendMessage(
    jid, 
    {
       ...
       mimetype: 'image/jpeg', 
       jpegThumbnail: await Rafz.resize('https://example.com/jdbenkksjs.jpg', 320, 320), 
       caption: 'Body',
       title: 'Title',
       subtitle: 'Subtitle', 
       footer: 'Footer',
...
await Rangga.sendMessage(
    ...
    { 
        album: [{
        	image: {
        		url: 'https://example.com/itRafzchan.jpg'
        	}, 
        	caption: 'Hay'
        }, {
        	image: Buffer, 
        	...
        	video: {
        		url: 'https://example.com/itRafzchan.mp4'
        	}, 
        	caption: 'Hay'
        }, {
        	video: Buffer, 
...
```ts
const msg = await Rangga.sendMessage(jid, { text: 'hello word' })
await Rangga.sendMessage(jid, { delete: msg.key })
```
...
import { downloadMediaMessage, getContentType } from '@Rafflie-CH/Baileys'

Rafz.ev.on('messages.upsert', async ({ [m] }) => {
    ...
    if (messageType === 'imageMessage') {
        ...
        const stream = await downloadMediaMessage(
            ...
            { 
                ...
                reuploadRequest: Rafz.updateMediaMessage
            }
        )
        // save to file
...
```ts
await Rafz.updateMediaMessage(msg)
```
...
```ts
await Rafz.rejectCall(callId, callFrom)
```
...
```ts
await Rafz.sendPresenceUpdate('available', jid) 
```

> [!NOTE]
> If a desktop client is active, WA doesn't send push notifications to the device. If you would like to receive said notifications -- mark your Baileys client offline using `Rafz.sendPresenceUpdate('unavailable')`
...
await Rafz.chatModify({ archive: true, lastMessages: [lastMsgInChat] }, jid)
```
### Mute/Unmute a Chat
...
```ts
// mute for 8 hours
await Rafz.chatModify({ mute: 8 * 60 * 60 * 1000 }, jid)
// unmute
await Rafz.chatModify({ mute: null }, jid)
```
### Mark a Chat Read/Unread
```ts
const lastMsgInChat = await getLastMessageInChat(jid) // implement this on your end
// mark it unread
await Rafz.chatModify({ markRead: false, lastMessages: [lastMsgInChat] }, jid)
```

### Delete a Message for Me
```ts
await Rafz.chatModify(
    {
        clear: {
            messages: [
                {
...
await Rangga.sendMessage(jid, { text: 'hello' }, { ephemeralExpiration: WA_DEFAULT_EPHEMERAL })
...
### Clear Messages
```ts
await Rafz.clearMessage(jid, key, timestamps) 
```
...
```ts
const [result] = await Rafz.onWhatsApp(jid)
...
await Rafz.fetchMessageHistory(
...
### Fetch Status
```ts
const status = await Rafz.fetchStatus(jid)
...
```ts
// for low res picture
const ppUrl = await Rafz.profilePictureUrl(jid)
console.log(ppUrl)
```

### Fetch Bussines Profile (such as description or category)
```ts
const profile = await Rafz.getBusinessProfile(jid)
...
Rafz.ev.on('presence.update', console.log)

// request updates for a chat
await Rafz.presenceSubscribe(jid) 
```
...
```ts
await Rafz.updateProfileStatus('Hello World!')
```
### Change Profile Name
```ts
await Rafz.updateProfileName('My name')
```
...
```ts
await Rafz.updateProfilePicture(jid, { url: './new-profile-picture.jpeg' })
```
### Remove display picture (groups too)
```ts
await Rafz.removeProfilePicture(jid)
```
...
```ts
// title & participants
const group = await Rafz.groupCreate('My Fab Group', ['1234@s.whatsapp.net', '4564@s.whatsapp.net'])
console.log('created group with id: ' + group.gid)
await Rangga.sendMessage(group.id, { text: 'hello there' }) // say hello to everyone on the group
```
### Add/Remove or Demote/Promote
```ts
// id & people to add to the group (will throw error if it fails)
await Rafz.groupParticipantsUpdate(
    jid, 
...
### Change Subject (name)
```ts
await Rafz.groupUpdateSubject(jid, 'New Subject!')
```
### Change Description
```ts
await Rafz.groupUpdateDescription(jid, 'New Description!')
```
### Change Settings
```ts
// only allow admins to send messages
await Rafz.groupSettingUpdate(jid, 'announcement')
// allow everyone to send messages
await Rafz.groupSettingUpdate(jid, 'not_announcement')
// allow everyone to modify the group's settings -- like display picture etc.
await Rafz.groupSettingUpdate(jid, 'unlocked')
// only allow admins to modify the group's settings
await Rafz.groupSettingUpdate(jid, 'locked')
```
### Leave a Group
```ts
// will throw error if it fails
await Rafz.groupLeave(jid)
```
### Get Invite Code
- To create link with code use `'https://chat.whatsapp.com/' + code`
```ts
const code = await Rafz.groupInviteCode(jid)
console.log('group code: ' + code)
```
### Revoke Invite Code
```ts
const code = await Rafz.groupRevokeInvite(jid)
console.log('New group code: ' + code)
```
### Join Using Invitation Code
- Code can't have `https://chat.whatsapp.com/`, only code
```ts
const response = await Rafz.groupAcceptInvite(code)
console.log('joined to: ' + response)
```
### Get Group Info by Invite Code
```ts
const response = await Rafz.groupGetInviteInfo(code)
...
```ts
const response = await Rafz.groupAcceptInviteV4(jid, groupInviteMessage)
console.log('joined to: ' + response)
```
### Get Request Join List
```ts
const response = await Rafz.groupRequestParticipantsList(jid)
console.log(response)
```
### Approve/Reject Request Join
```ts
const response = await Rafz.groupRequestParticipantsUpdate(
    jid, // group id
    ['abcd@s.whatsapp.net', 'efgh@s.whatsapp.net'],
    'approve' // or 'reject' 
)
...
```ts
const response = await Rafz.groupFetchAllParticipating()
console.log(response)
```
### Toggle Ephemeral
...
```ts
await Rafz.groupToggleEphemeral(jid, 86400)
```

### Change Add Mode
```ts
await Rafz.groupMemberAddMode(
    jid,
...
### Block/Unblock User
```ts
await Rafz.updateBlockStatus(jid, 'block') // Block user
await Rafz.updateBlockStatus(jid, 'unblock') // Unblock user
```
### Get Privacy Settings
```ts
const privacySettings = await Rafz.fetchPrivacySettings(true)
console.log('privacy settings: ' + privacySettings)
```
### Get BlockList
```ts
const response = await Rafz.fetchBlocklist()
console.log(response)
```
### Update LastSeen Privacy
```ts
const value = 'all' // 'contacts' | 'contact_blacklist' | 'none'
await Rafz.updateLastSeenPrivacy(value)
```
### Update Online Privacy
```ts
const value = 'all' // 'match_last_seen'
await Rafz.updateOnlinePrivacy(value)
```
### Update Profile Picture Privacy
```ts
const value = 'all' // 'contacts' | 'contact_blacklist' | 'none'
await Rafz.updateProfilePicturePrivacy(value)
```
### Update Status Privacy
```ts
const value = 'all' // 'contacts' | 'contact_blacklist' | 'none'
await Rafz.updateStatusPrivacy(value)
```
### Update Read Receipts Privacy
```ts
const value = 'all' // 'none'
await Rafz.updateReadReceiptsPrivacy(value)
```
### Update Groups Add Privacy
```ts
const value = 'all' // 'contacts' | 'contact_blacklist'
await Rafz.updateGroupsAddPrivacy(value)
```
...
await Rafz.updateDefaultDisappearingMode(ephemeral)
```
...
```ts
const bList = await Rafz.getBroadcastListInfo('1234@broadcast')
...
Rafz.ws.on('CB:edge_routing', (node: BinaryNode) => { })

// for any message with tag 'edge_routing' and id attribute = abcd
Rafz.ws.on('CB:edge_routing,id:abcd', (node: BinaryNode) => { })

// for any message with tag 'edge_routing', id attribute = abcd & first content node routing_info
Rafz.ws.on('CB:edge_routing,id:abcd,routing_info', (node: BinaryNode) => { })
```
